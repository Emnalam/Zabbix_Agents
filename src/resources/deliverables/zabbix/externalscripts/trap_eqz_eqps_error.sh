...
ftp_file_pattern=$(grep -l "<subproduct>eqps</subproduct>" $ftp_file_pattern xx 2>/dev/null)
...
for f in ${ftp_file_pattern}; do
  ...
  zabbix_sender -z 127.0.0.1 -s "$CLIENT - $PRODUCT" -k "trap.eqz_eqps_error" -o "$ECHO"
