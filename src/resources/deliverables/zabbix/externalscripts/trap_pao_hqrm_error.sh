...
ftp_file_pattern=$(grep -l "<subproduct>hqrm</subproduct>" $ftp_file_pattern xx 2>/dev/null)
...
for f in ${ftp_file_pattern}; do
  ...
  rm -rf $f
	zabbix_sender -z 127.0.0.1 -s "$CLIENT - $PRODUCT" -k "trap.pao_hqrm_error" -o "$ECHO"
done
