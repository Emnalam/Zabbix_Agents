...
ftp_file_pattern=$(grep -l "<subproduct>appia</subproduct>" $ftp_file_pattern xx 2>/dev/null)

for f in ${ftp_file_pattern}; do
  agent=`awk -F'[<|>]' '/agent_Host/{printf "%s", $3}' $f`
  agentDate=`awk -F'[<|>]' '/agentExecutionDate/{printf "%s", $3}' $f`
  code=`awk -F'[<|>]' '/agentType/{printf "%s", $3}' $f`
  ...
  rm -rf $f
	zabbix_sender -z 127.0.0.1 -s "$CLIENT - $PRODUCT" -k "trap.apsys_appia_error" -o "$ECHO"
done
